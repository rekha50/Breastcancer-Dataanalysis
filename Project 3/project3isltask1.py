...
Clearly, there must be a tradeoff between margin and #exceptions when selecting
the optimal C (often based on cross validation).
'''
...
roc_rbf= plot_roc_curve(svc_classifier, x_train, y_train)
plt.show()


#ROC curve for testing data:
from sklearn.metrics import plot_roc_curve
roc_rbf = plot_roc_curve(svc_classifier, x_test, y_test)
plt.show()
...
svc_classifier = svm.SVC(kernel='poly', C=1.0, degree=2).fit(x_train,y_train)
print(svc_classifier.predict(x_test))
print(svc_classifier.score(x_test,y_test))

#ROC curve for training data:
from sklearn.metrics import  plot_roc_curve
roc_lda = plot_roc_curve(svc_classifier, x_train, y_train)
plt.show()


#ROC curve for testing data:
from sklearn.metrics import plot_roc_curve
roc_lda = plot_roc_curve(svc_classifier, x_test, y_test)
plt.show()
...
svc_classifier = svm.SVC(kernel='poly', C=1.0, degree=3).fit(x_train,y_train)
print(svc_classifier.predict(x_test))
print(svc_classifier.score(x_test,y_test))
